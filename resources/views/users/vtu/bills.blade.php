...
@section('main')
    ...
    <section class="w-full column g-10">
        {{-- topup form --}}
        <form action="{{ url('users/post/electricity/bill/process') }}" method="POST" onsubmit="PostRequest(event,this,Completed)" style="box-shadow:0px 0px 10px rgba(0,0,0,0.1)" class="w-full br-20 column g-10 p-20 bg-light">
           {{-- csrf token --}}
           ...
           {{-- trx pin --}}
            ...
            <div class="column g-5 w-full">
                <label>Amount</label>
                <div class="cont">
                    <input oninput="CalculateTotal(this)" type="number" name="amount" placeholder="Enter amount(min: ₦1,000)" inputmode="numeric" class="inp input required">
                </div>
            </div>
             {{-- new input --}}
            <div class="column g-5 w-full">
           ...
           <div onclick="OpenConfirmationModal(this)" class="w-full btn br-1000 row align-center border-none justify-center bg-primary primary-text no-selecvt pc-pointer m-top-10 p-10 h-50 g-10">Pay Bill</div>
        </form>
    </section>
    {{-- confirmation modal --}}
