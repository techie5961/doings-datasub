...
@section('main')
    <section class="w-full column g-10">
       ...
       <template class="network-{{ $details[0]['ID'] }}">
        ...
        {{-- topup form --}}
        <form action="{{ url('users/post/data/topup/process') }}" method="POST" onsubmit="PostRequest(event,this,Completed)" style="box-shadow:0px 0px 10px rgba(0,0,0,0.1)" class="w-full br-20 column g-10 p-20 bg-light">
           {{-- csrf token --}}
           ...
           {{-- trx pin --}}
            ...
            <div class="w-full grid grid-2 pc-grid-4 g-10">
                @foreach (MobileNetworks() as $name => $network)
                    <div onclick="ChooseNetwork(this,'{{ $name }}',' {{ $network->api_code }}')" class="network">
            ...
            <div class="column display-none data-plans w-full g-5">
                ...
                <div class="cont">
                <select onchange="DataPlanSelected(this)" name="plan" class="inp required input">
           ...
           <div onclick="OpenConfirmationModal(this)" class="w-full btn br-1000 row align-center border-none justify-center bg-primary primary-text no-selecvt pc-pointer m-top-10 p-10 h-50 g-10">Buy Data</div>
        </form>
    </section>
    {{-- confirmation modal --}}
...
@section('js')
    <script class="js">
            ...
            function ChooseNetwork(element,network_key,api_code){
           ...
           function  OpenPinModal(){
            ...
            function DataPlanSelected(element){
