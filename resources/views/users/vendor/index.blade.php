    ...
    <section class="w-full column g-10">
           ...
           {{-- new row --}}
            <div class="row w-full g-10 align-center space-between">
                ...
                <div class="column g-5">
                    <strong class="font-1 opacity-07">Vendor Plan</strong>
                    @if (($settings->vendor ?? 0) != 0)
                        <div style="align-items:flex-end" class="row g-5">
                            <strong class="desc">{{ $currency }}{{ number_format($settings->vendor,2) }}</strong>
                            <span>
                                / one-time
                            </span>
                        </div>
            ...
            <small class="opacity-o5">Get access to vendor-specific tools and higher transaction limits.</small>
        </div>
        {{-- what you get --}}
        <div style="box-shadow: 0 0 10px rgba(0,0,0,0.1)" class="column bg-light br-10 p-20 g-10">
            ...
            <div class="row w-full g-5">
                ...
                <span>Access to vendor reports</span>
            </div>
             {{-- new row --}}
            <div class="row w-full g-5">
                <span class="c-green">
                ...
                <span>Vendor tools & tutorials</span>
            </div>
        </div>
        {{-- upgrade btn --}}
       @if (Auth::guard('users')->user()->type == 'vendor')
            <div style="pointer-events: none;filter:grayscale(50%)" onclick="document.querySelector('.modal').classList.add('active')" class="w-full br-10 p-10 font-1 bold row h-50 bg-primary primary-text align-center justify-center no-select pointer">Account already upgraded to vendor</div>
      
       @else
        <div onclick="document.querySelector('.modal').classList.add('active')" class="w-full br-10 p-10 font-1 bold row h-50 bg-primary primary-text align-center justify-center no-select pointer">Upgrade to Vendor</div>
    ...
    <section onclick="this.classList.remove('active')" class="modal">
        <div onclick="event.stopPropagation()" class="child">
           ...
           {{-- prompt --}}
            ...
            <p>You are about to upgrade to an Vendor Account.You can view our pricing page for details about the discounts available for Vendors.</p>
            <p>You would be charged {{ $currency }}{{ ($settings->vendor ?? 0) == 0 ? 0 : number_format($settings->vendor,2)}} for this service. To continue, enter your transaction pin below</p>
            </div>
            {{-- upgrade form --}}
            <form action="{{ url('users/post/upgrade/account/process') }}" method="POST" onsubmit="PostRequest(event,this,MyFunc.Upgraded)" class="w-full column g-10">
               ...
               {{-- upgrade type --}}
               <input type="hidden" class="inp input" name="type" value="vendor">
                {{-- new input --}}
