...
@section('main')
    <section class="w-full flex-auto align-center justify-center column g-10">
        {{-- form --}}
        <form method="POST" onsubmit="PostRequest(event,this,MyFunc.LoggedIn)" action="{{ url('admins/post/login/process') }}" style="border:1px solid var(--rgt-01)" class="w-full align-center column g-10 br-20 p-20 bg-light max-w-500">
        {{-- logo image --}}
            <img src="{{ asset('logos/'.config('settings.logo').'') }}" alt="Site Logo" class="h-70">
