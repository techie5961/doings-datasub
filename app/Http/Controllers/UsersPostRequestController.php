...
class UsersPostRequestController extends Controller
{
  ...
  DB::table('transactions')->insert([
    ...
    public function UpgradeAccount(){
       ...
       $cost=$settings->{request('type')} ?? 0;
        if(!Hash::check(request('pin'),Auth::guard('users')->user()->pin)){
        return response()->json([
            'message' => 'Invalid transaction pin',
            'status' => 'error'
        ]);
        ...
        if($cost != 0){
            if($cost > Auth::guard('users')->user()->main_balance){
                return response()->json([
    ...
    'balance' => [
        'before' => Auth::guard('users')->user()->main_balance,
        'after' => Auth::guard('users')->user()->main_balance - $cost
    ],
    'primary_wallet' => 'Main Wallet'
